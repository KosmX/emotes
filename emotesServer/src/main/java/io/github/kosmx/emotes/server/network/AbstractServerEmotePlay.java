...
public abstract class AbstractServerEmotePlay<P extends IServerNetworkInstance> extends ServerEmoteAPI {
    ...
    protected void stopEmote(P player, @Nullable NetData originalMessage) {
        PlayingAnimationData emote = player.getEmoteTracker().getPlayedEmote();
        player.getEmoteTracker().setPlayedEmote(null);
        if (emote != null) {
    ...
    public void playerStartTracking(P tracked, P tracker) {
        ...
        PlayingAnimationData playedEmote = tracked.getEmoteTracker().getPlayedEmote();
        if (playedEmote != null) {
            sendForPlayer(playedEmote.preparePacket().configureTarget(getUUIDFromPlayer(tracked)).build().data, tracked, tracker);
        }
    }
